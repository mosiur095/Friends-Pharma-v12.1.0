...
class GetUserUseCase @Inject constructor(private val apiRepo: ApiRepo) {

    
    operator fun invoke(id: String): Flow<Async<UserDetailsDto>> = flow {
        try {
            emit(Async.Loading())
            val order = apiRepo.getUser(id)
