...
class TrackOrderUseCase  @Inject constructor(private val apiRepo: ApiRepo) {

    
    operator fun invoke(id: String): Flow<Async<TrackOrderDto>> = flow {
        try {
            emit(Async.Loading())
            val track = apiRepo.trackOrder(id)
