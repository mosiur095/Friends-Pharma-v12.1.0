...
class GetOrdersUseCase @Inject constructor(private val apiRepo: ApiRepo) {
    
    operator fun invoke(mobile: String): Flow<Async<OrdersDto>> = flow {
        try {
            emit(Async.Loading())
            val order = apiRepo.getOrders(mobile)
