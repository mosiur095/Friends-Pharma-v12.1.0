...
class GetAddressUseCase @Inject constructor(private val apiRepo: ApiRepo) {
    
    operator fun invoke(id: String): Flow<Async<AddressDto>> = flow {
        try {
            emit(Async.Loading())

            val address = apiRepo.getAddress(id)
