...
class ProductReturnUseCase @Inject constructor(private val apiRepo: ApiRepo) {

    
    operator fun invoke(id: String): Flow<Async<ReturnProductDto>> = flow {
        try {
            emit(Async.Loading())
            val submit = apiRepo.returnProduct(id)
